...
#eval=FALSE = don’t run this code; just show it
...
#gzcon = Treat the incoming data as gzipped and decompress it before reading
```
...
tidy_tweets %>%
  ...
  full_join(totals, word, by="date") %>% #merges the two datasets by date and keeps all dates from both tables, filling missing values with NA #full_join creates a brand‑new tibble 
...
#@x -> a numeric vector containing all non‑zero values of the dfm, stored column‑by‑column
#to read the dfm -> as.matrix(dfmat_news_lsd), number of rows and column -> dim(dfmat_news_lsd)
date <- dfmat_news_lsd@Dimnames$docs #the names of the rows
...
unique(docvars(toks_news)$username) #From the metadata attached to each document, extract the username column and show me all the distinct entries
```
...
tidy_dfm_news_lsd %>%
  ...
  geom_point(aes(x=reorder(newspaper, -sentiment), y=sentiment)) + # reordering (sorts the newspapers by sentiment) newspaper variable so it is displayed from most negative to most positive
  coord_flip() + # pivot plot by 90 degrees -> making it newspaper on x-axis and sentiment on y-axis
...
tidy_dfm_news_lsd <- tidy_dfm_news_lsd %>%
  extract(doc_id, into = c("newspaper", "date"), regex = "([a-zA-Z]+)\\.(.+)") %>% #splits the combined doc_id into newspaper and date using a regex
...
tidy_tweets %>%
  mutate(obs=1) %>%
  filter(grepl(paste0(trans_words, collapse = "|"), word, ignore.case = T)) %>% #filtered out the 0 tokens
  group_by(newspaper) %>%
  summarise(sum_mwords = sum(obs)) %>%
  full_join(totals_newspaper, by="newspaper") %>%
  mutate(sum_mwords= ifelse(is.na(sum_mwords), 0, sum_mwords), #cuz by combining the data frame, some rows may contain NA in the filtered one as we discussed
...
dfm_dict_trans %>%
  mutate(
    ...
    terf  = ifelse(is.na(terf), 0, terf),
    sum_words = ifelse(is.na(sum_words), 0, sum_words)
  ) %>%
  mutate(
    pct_trans = trans / sum_words,
    pct_terf  = terf  / sum_words
  ) %>%
  pivot_longer(cols = c(pct_trans, pct_terf),
               names_to = "category",
               values_to = "pct") %>%
  ggplot(aes(x = reorder(newspaper, -pct), y = pct, color = category)) +
  geom_point() +
  coord_flip() +
  xlab("Newspaper") +
  ylab("Percentage of dictionary matches") +
  theme_minimal()
...
```{r}
#plot by tidytext
trans_words <- c("trans", "transgender", "lgbtq", "lgbtq+") 
terf_words <- c("terf", "terfs", "transphobic", "transphobia")
all_words <- c(trans_words, terf_words)
...
tidy_tagged <- tidy_tweets %>%
  mutate(
    category = case_when(
      word %in% trans_words ~ "trans",
      word %in% terf_words  ~ "terf",
      TRUE ~ NA_character_
    )
  )

#Count matches per newspaper and category
dict_counts <- tidy_tagged %>%
  filter(!is.na(category)) %>%
  group_by(newspaper, category) %>%
  summarise(count = n(), .groups = "drop")

#join totals + compute percentages
dict_plot_data <- dict_counts %>%
  left_join(totals_newspaper, by = "newspaper") %>%
  mutate(pct = count / sum_words)
...
dict_plot_data %>%
  ...
  coord_flip() +
  xlab("Newspaper") +
  ylab("% of words matching dictionary") +
  theme_minimal()
...
```{r}
tweets$newspaper <- tweets$username
tweet_corpus <- corpus(tweets, text_field = "tweet", docvars = "newspaper")

toks <- tokens(tweet_corpus, remove_punct = TRUE)

#apply lexicoder
dfm_sent <- dfm(toks) %>% 
  dfm_lookup(dictionary = data_dictionary_LSD2015)

#group by newspaper and convert
dfm_sent_news <- dfm_sent %>%
  dfm_group(groups = newspaper)

tidy_dfm_news <- dfm_sent_news %>%
  convert(to = "data.frame") %>%
  rename("newspaper" = doc_id) %>% # when converting to data.frame, R called our grouping variable 'doc_id'. We rename it 'newspaper' instead.
  mutate(sentiment = positive - negative) # create variable for overall sentiment
...
#positive vs negative sentiment per newspaper
tidy_dfm_news %>%
  pivot_longer(cols = c(positive, negative),
               names_to = "sentiment_type",
               values_to = "count") %>%
  ggplot(aes(x = reorder(newspaper, -count), y = count, color = sentiment_type)) +
  geom_point(size = 3) +
  coord_flip() +
  theme_minimal() +
  xlab("Newspaper") +
  ylab("Lexicoder sentiment word counts")

#Net sentiment (positive − negative) per newspaper
tidy_dfm_news %>%
  ggplot(aes(x = reorder(newspaper, -sentiment), y = sentiment)) +
  geom_point(size = 3) +
  coord_flip() +
  theme_minimal() +
  xlab("Newspaper") +
  ylab("Net sentiment (positive − negative)")
